...
c) Abandoned Build Clause: If a build is incomplete and filed under the [projects orphanage](https://westeroscraft.com/wiki/guides/projects-orphanage) as an abandoned build, it may be allowed to be redone. The intuition behind this clause is that it can be difficult to pick up someone else's outdated work, and if we expect people to do this very few people would want to apply to finish abandoned builds. However, moderators reserve the right to veto a redo for an abandoned build if they deem the existing portions to be particularly high quality.
